...
8. **Use MAUI `NavigationPage.BarBackgroundColorProperty` and `NavigationPage.BarTextColorProperty` values on DUI `ContentPage`** when a modal `NavigationPage` needs page-specific navigation bar colors. Set them with `SetValue(NavigationPage.BarBackgroundColorProperty, color)` / `SetValue(NavigationPage.BarTextColorProperty, color)` because MAUI does not expose XAML attachable accessors for these properties. `BarBackgroundColor` controls the modal navigation bar background; `BarTextColor` controls the modal navigation title, back button, overflow icon, and toolbar item tint.
9. **Don't** apply `ContentPage` navigation bar appearance from `Loaded`; keep `Loaded` for timer/logging work if needed, but use the handler lifecycle (`OnHandlerChanged`/`HandlerChanged`) and Shell appearance trackers for native navigation appearance so native resources are available and stable.
10. **For modal navigation bar colors**, DUI `ContentPage` should set unset `NavigationPage.BarBackgroundColorProperty` / `NavigationPage.BarTextColorProperty` values from Shell navigation bar colors and copy them to its parent `NavigationPage` from `OnParentSet`. This gives the wrapper the correct first-frame color before `PushModalAsync`. Prefer relying on MAUI's own `NavigationPage` behavior for subsequent pushed pages before adding `OnAppearing`, `OnNavigatedTo`, handler lifecycle, renderer subscriptions, or native status-bar refresh logic.
11. **For iOS modal status bar text contrast**, first test MAUI's default `NavigationPage` handler by requesting `SetNeedsStatusBarAppearanceUpdate()` after the page-level bar colors are copied. Do not register `ModalNavigationRenderer` unless the default handler demonstrably cannot update status bar text contrast from the refreshed navigation bar state.
12. **Don't** use `OverrideUserInterfaceStyle` or `ModalPresentationCapturesStatusBarAppearance` to force iOS modal status bar text contrast from the navigation bar color. Don't change translucency, appearance presets, shadow visibility, or native bar tinting unless a concrete regression requires it.
13. **Don't** start or repeat a build when diagnostics already show a compile error. Fix the diagnostic first, verify errors are clean, then run the focused build once.
14. **When reusing an existing component in a new context** (e.g. embedding a component inside a new handler, effect, or renderer), always read the component's existing canonical platform consumer first and replicate **all** of its event subscriptions, lifecycle hooks, and teardown logic. Never assume that rendering the component is sufficient. Components often have additional contracts (update events, binding context propagation, etc.) that only the canonical consumer reveals. Missing these causes silent regressions where the component renders correctly initially but fails to update afterwards.
