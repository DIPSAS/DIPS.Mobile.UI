...
10. **For `ContentPage` modal navigation bar colors**, `NavigationBarColor` and `NavigationBarTextColor` are page-specific overrides. When unset, resolve `NavigationPage.BarBackgroundColor` / `NavigationPage.BarTextColor` first, then Shell attached colors from the modal page/navigation context before `Shell.Current`, then design defaults. Use one resolved text color for title, back button tint, overflow icon tint, and toolbar item text on both iOS and Android.
