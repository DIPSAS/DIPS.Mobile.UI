...
using DIPS.Mobile.UI.Droid.Components.ContextMenus;
using Xamarin.Forms;
...
namespace DIPS.Mobile.UI.Droid.Components.ContextMenus
{
        ...
        PopupMenu.IOnMenuItemClickListener, Application.IActivityLifecycleCallbacks
    {
        ...
        private void OpenContextMenu(object sender, EventArgs e)
        {
            ...
            var popupMenu = new PopupMenu(Context, Control);
            m_menuItems = ContextMenuHelper.CreateMenuItems(m_contextMenuButton.ItemsSource,
            ...
            m_menuHelper.SetForceShowIcon(m_menuItems.Keys.Any(contextMenuItem =>
                !string.IsNullOrEmpty(contextMenuItem.Icon) ||
                !string.IsNullOrEmpty(contextMenuItem.AndroidOptions
                    .IconResourceName))); //Show icons if there is any context menu items with a icon added
            m_menuHelper.Show();
