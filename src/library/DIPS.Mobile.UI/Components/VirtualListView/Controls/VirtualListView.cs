...
public partial class VirtualListView : View, IVirtualListView, IVirtualListViewSelector
{
    ...
    public static readonly BindableProperty UnfocusViewProperty = BindableProperty.Create(
        nameof(ViewToUnfocusOnScrolled),
        typeof(View),
        typeof(VirtualListView));

    /// <summary>
    /// Will unfocus this view when the list is scrolled
    /// </summary>
    public View? ViewToUnfocusOnScrolled
    {
        get => (View?)GetValue(UnfocusViewProperty);
        set => SetValue(UnfocusViewProperty, value);
    }
	...
	public void Scrolled(double x, double y)
	{
        ...
        if(ViewToUnfocusOnScrolled is null)
            return;
	...
	public IView CreateView(PositionInfo position, object data)
		...
		{
			PositionKind.Item =>
				ItemTemplateSelector?.SelectTemplate(data, position.SectionIndex, position.ItemIndex, this)?.CreateContent() as View
					?? ItemTemplate?.CreateContent() as View,
			PositionKind.SectionHeader =>
				SectionHeaderTemplateSelector?.SelectTemplate(data, position.SectionIndex, this)?.CreateContent() as View
					?? SectionHeaderTemplate?.CreateContent() as View,
			PositionKind.SectionFooter =>
				SectionFooterTemplateSelector?.SelectTemplate(data, position.SectionIndex, this)?.CreateContent() as View
	...
	public string GetReuseId(PositionInfo position, object data)
		=> position.Kind switch
		{
			PositionKind.Item =>
				"ITEM_" +
					(ItemTemplateSelector?.SelectTemplate(data, position.SectionIndex, position.ItemIndex, this)
						?? ItemTemplate)?.GetDataTemplateId() ?? "0",
			PositionKind.SectionHeader =>
				"SECTION_HEADER_" +
					(SectionHeaderTemplateSelector?.SelectTemplate(data, position.SectionIndex, this)
						?? SectionHeaderTemplate)?.GetDataTemplateId() ?? "0",
			PositionKind.SectionFooter =>
				"SECTION_FOOTER_" +
					(SectionFooterTemplateSelector?.SelectTemplate(data, position.SectionIndex, this)
