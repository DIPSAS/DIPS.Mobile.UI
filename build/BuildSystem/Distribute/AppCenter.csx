...
using DIPS.Buildsystem.Core.Http;
...
public static class AppCenter
{
    ...
    public static async Task<List<string>> DistributeRelease(int releaseId, string appGroup, string[] distributionGroups, string apiToken, string releaseNotes)
    {
        ...
        var restClient = RestClient.GetInstance(AppCenterApiUrl);
    ...
    {
        var fileRestClient = RestClient.GetInstance(AppCenterFileUrl);
    ...
    {
        var apiRestClient = RestClient.GetInstance(AppCenterApiUrl);
        var relativeUrl = $"{Path.Combine(OwnerName, appGroup)}/uploads/releases/{metadata.Id}";
    ...
    {
        var restClient = RestClient.GetInstance(AppCenterFileUrl);
