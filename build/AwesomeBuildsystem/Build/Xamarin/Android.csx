...
public static class Android
{
    ...
    /// <returns></returns>
    public static Task Build(string androidProjectPath, string configuraiton ="Release"){
        var androidProjectFilePath = FileHelper.FindSingleFileByExtension(androidProjectPath,".csproj").FullName;
        return Command.ExecuteAsync("msbuild", $"{androidProjectFilePath} /p:Configuration={configuraiton}");
    }
    ...
    /// <remarks>When creating an Android package it must be signed with a keystore. Please contact Team Mobil to get the keystore details.</remarks>
     public static async Task Package(
        ...
        //Package the app, results in an .apk
        await Command.ExecuteAsync("msbuild", $"{androidProjectFilePath} /p:Configuration={configuration} /t:PackageForAndroid");

        //Make sure there is one, and only one .apk in the bin configuration folder
        var apkFile = FileHelper.FindSingleFileByExtension(Path.Combine(androidProjectPath, "bin", configuration), ".apk");
        var apkFilePath = apkFile.FullName;

        //What is zipalign? https://developer.android.com/studio/command-line/zipalign
        if(shouldZipAlign)
        {
        ...
        Console.WriteLine($"Copy {apkFile.Name} to {outputDirectory}");
        var outPutDir = outputDirectory.EndsWith('/') ? outputDirectory : outputDirectory+"/";
        var newApkFilePath = outPutDir+apkFile.Name;
        File.Copy(apkFilePath, newApkFilePath, true);
    }
