...
public static class Command
{
    ...
    public static async Task ExecuteAsync(string commandPath, string arguments, string workingDirectory = null, int success = 0, bool echo = true)
    {
        Console.WriteLine($"##[command]Executing command {commandPath} {arguments} in working directory {workingDirectory}");
        var process = CreateProcess(commandPath, arguments, workingDirectory);
        RedirectToConsole(process);
        var exitCode = await StartProcessAsync(process, echo);
        if (exitCode != success)
        {
