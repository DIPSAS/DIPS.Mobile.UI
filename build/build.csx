...
AsyncStep createResourcesPR = async () =>
{
    ...
    await Command.CaptureAsync("echo", $"'{ghPAT}' >> {ghTokenFilePath}");
    await Command.ExecuteAsync("gh", $"auth login --with-token < {ghTokenFilePath}");
    ...
    await Command.ExecuteAsync("gh", "auth logout"); //Clean up after we're done
};
