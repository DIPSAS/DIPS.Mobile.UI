...
AsyncStep createResourcesPR = async () =>
{
    ...
    var ghPAT = AzureDevops.GetEnvironmentVariable("GithubPAT");
    await Command.CaptureAsync("echo", $"'{ghPAT}' >> {ghTokenFilePath}", logCommand:false);
    await Command.CaptureAsync("gh", $"auth login --with-token < {ghTokenFilePath}");
    File.Delete(ghTokenFilePath); //Clean up after we're done
    await Command.ExecuteAsync("gh", $"pr create --title \"{changesetMessage}\n --body \"Here is the latest resources that was generated by DIPS.Mobile.DesignTokens repository\"");
};
